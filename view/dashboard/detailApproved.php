...
<head>
    ...
    <title>Approved | Dash C-Trans</title>
...
<body>
    ...
    ?>
        ...
        <div class="w3-sidebar w3-bar-block w3-card w3-animate-left" style="display:none; background-color: #FF6000;" id="mySidebar">
            ...
            <a href="../../controller/page?page=Approved" class="w3-bar-item px-3 py-3 active-sidebar-active text-white" style="text-decoration: none;">Approved</a>
        </div>
    <?php
    } else {
    ?>
        ...
        <div class="w3-container">
            <div class="row my-3">
                <div class="col-lg-12 shadow-lg px-5 py-5 rounded-4">
                    <div class="my-4">
                        <span class="title-form">Detail Booking</span>
                    </div>
                    ...
                    <form action="">
                        <div class="row">
                            ...
                            $data = mysqli_query($conn, "SELECT * from tb_pemesanan WHERE idPemesanan='$id'");
                            foreach ($data as $row) {
                            ?>

                                <div class="col-lg-12">
                                    <div class="mb-3 shadow px-3 py-3">
                                        <label for="" style="color: #FF6000;">Pemesan : <?= $row['namaPemesanan']; ?></label>
                                        <p for="" style="color: #FF6000; font-weight:800; font-size:30px;">Type / Merk : <?= $row['namaKendaraan']; ?></p>
                                        <p>Booking : <?= $row['tglPemesanan']; ?></p>
                                        <p>Flat Nomor : <?= $row['flatNo']; ?></p>
                                        <p>Jenis Kendaraan : <?= $row['jenisKendaraan']; ?></p>
                                        <p>Status Kendaraan : <?= $row['statusKendaraan']; ?></p>
                                        <p>Jenis BBM : <?= $row['jenisBBM']; ?></p>
                                        <p>harga BBM : <?= $row['jmlBBM']; ?> Liter</p>
                                        <p>Total : Rp. <?= number_format($row['totalBBM']); ?></p>
                                        <?php
                                        if ($_SESSION['status'] == "Approval1") {
                                        ?>
                                            <a href="approved1" style="text-decoration: none; background-color:#FFA559; padding:10px; border-radius:25px; color:#fff;">Back</a>
                                        <?php
                                        } else if ($_SESSION['status'] == "Approval2") {
                                        ?>
                                            <a href="approved2" style="text-decoration: none; background-color:#FFA559; padding:10px; border-radius:25px; color:#fff;">Back</a>
                                        <?php
                                        } else {
                                        ?>
                                            <a href="approved3" style="text-decoration: none; background-color:#FFA559; padding:10px; border-radius:25px; color:#fff;">Back</a>
                                        <?php
                                        }
                                        ?>
